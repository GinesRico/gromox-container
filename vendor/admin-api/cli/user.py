...
def cliUserProvision(args):
    """Create the mailbox for an existing user without changing its identity."""
    cli = args._cli
    cli.require("DB")
    from orm import DB
    ...
    if ret:
        return ret
    if user.maildir:
        cli.print("User already has a mailbox: {}".format(user.maildir))
        _dumpUser(cli, user)
        return 0

    try:
        user.homeserverID, user.maildir = Servers.allocUser(user.ID, args.homeserver)
        with UserSetup(user, DB.session) as setup:
            setup.run()
        if not setup.success:
            DB.session.rollback()
            cli.print(cli.col("Could not provision mailbox: "+str(setup.error), "red"))
            return 1
        DB.session.commit()
        try:
            user.syncStore()
        ...
        DB.session.rollback()
        cli.print(cli.col("Could not provision mailbox: "+" - ".join(str(arg) for arg in err.args), "red"))
        return 1
...
def _setupCliUser(subp: ArgumentParser):
    ...
    provision = sub.add_parser("provision", help="Create a mailbox for an existing user")
    provision.add_argument("userspec", help="User ID or name prefix").completer = _cliUserspecCompleter
    provision.add_argument("--homeserver", type=int, metavar="ID", help="ID of the home server or 0 for local user")
    provision.set_defaults(_handle=cliUserProvision)
