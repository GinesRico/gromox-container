...
class Users(DataModel, DB.Base, NotifyTable):
    ...
    def checkCreateParams(data, maildir=True):
        ...
        # Límite propio de nuestra distribución.
        # 0 = sin límite.
        max_users = int(os.getenv("MAX_USERS", "0"))

        if (
            maildir
            and data.get("status", Users.NORMAL) == Users.NORMAL
            and max_users > 0
            and Users.count() >= max_users
        ):
            return "Maximum number of users reached"

        if "domainID" in data:
            domain = Domains.query.filter(
                Domains.ID == data.get("domainID")
            ).first()

        elif "@" in data["username"]:
            domain = Domains.query.filter(
                Domains.domainname == data["username"].split("@")[1]
            ).first()
        ...
        if data.get("status") != Users.CONTACT:
            domainUsers = Users.count(
                Users.domainID == domain.ID
            )
    ...
    def status(self, val):
        ...
        # self.status != NORMAL y val == NORMAL:
        # estamos reactivando un usuario.
        if (
            self.status
            and not val
            and max_users > 0
        ...
        self.addressStatus = (
            (self.addressStatus or 0) & ~self.USER_MASK
        ) | (val & self.USER_MASK)
