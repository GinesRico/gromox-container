...
const AddUser = (props: AddUserProps) => {
  ...
  const handleAdd = () => {
    const { username, password, properties, status, homeserver, chat, lang, noMaildir } = state;
    ...
    // eslint-disable-next-line camelcase
      { smtp: noMaildir ? false : smtp, pop3_imap: noMaildir ? false : pop3_imap,
        changePassword, privChat, privVideo, privFiles, privArchive }
      : {};
    ...
    dispatch(addUserData(domain.ID, {
      ...
      status,
      homeserver: noMaildir ? null : homeserver?.ID || null,
      properties: {
        ...properties,
  ...
  const handleAddAndEdit = () => {
    ...
    const checkboxes = status !== USER_STATUS.SHARED ? {
      ...CreateParams.user,
      smtp: noMaildir ? false : CreateParams.user.smtp,
      pop3_imap: noMaildir ? false : CreateParams.user.pop3_imap,
    } : {};
    setState({ ...state, loading: true });
    dispatch(addUserData(domain.ID, {
      username,
  ...
  const { username, loading, properties, password, repeatPw,
    status, homeserver, lang, chat, chatAvailable, noMaildir } = state;
      ...
      <DialogContent>
        <FormControl className={classes.form}>
          ...
          <FormControlLabel
            control={
              <Checkbox
                checked={noMaildir || false}
                onChange={handleCheckbox('noMaildir')}
                color="primary"
              />
            }
            label={t('Operator without own mailbox')}
            disabled={status === USER_STATUS.SHARED}
            className={classes.input}
          />
          <TextField 
            label={t("Username")}
          ...
          {isSystemAdmin && !noMaildir && <MagnitudeAutocomplete<Server>
            value={homeserver}
          ...
          <FormControlLabel
            ...
            disabled={!chatAvailable || noMaildir}
          />
        </FormControl>
      </DialogContent>
      <DialogActions>
